...
const COMPONENT_FIELD = "__m_component";
...
const createComponent = (fn, props, key) => {
  ...
  const component = hook(
    () => {
      ...
      const newVNode = Array.isArray(ret) ? h(
        "_",
...
const styleObjectToCss = (styleObject) => {
  let str = "";
  for (const prop in styleObject) {
    ...
    if (value !== null && value !== "") {
      if (str)
        str += " ";
      str += prop.startsWith("-") ? prop : JS_TO_CSS[prop] || (JS_TO_CSS[prop] = prop.replace(CSS_REGEX, "-$1").toLowerCase());
...
const fromVNodeToString = (vnode) => {
  ...
  for (let prop in vnode.props) {
    if (prop === "key" || prop === "ref" || prop === "children" || prop.startsWith("on")) {
      continue;
    }
    ...
    if (prop === "for")
      prop = "htmlFor";
    if (prop === "style")
      propValue = styleObjectToCss(propValue);
